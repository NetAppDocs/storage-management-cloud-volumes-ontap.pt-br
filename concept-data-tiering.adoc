...
Ao ativar o armazenamento em camadas no Google Cloud, Cloud Volumes ONTAP usa discos persistentes como camada de desempenho para dados acessados com frequência e um bucket do Google Cloud Storage como camada de capacidade para dados inativos. Você pode selecionar uma camada de armazenamento ao criar um sistema e alterá-la a qualquer momento depois. Para obter informações sobre como alterar a classe de armazenamento, consulte link:task-tiering.html["Dados inativos em camadas para armazenamento de objetos de baixo custo"].
...
--
Classes de armazenamento:: O Google Cloud utiliza  https://cloud.google.com/storage/docs/storage-classes["classes de armazenamento"^] para gerenciar custos e disponibilidade de dados. Recomendamos que você selecione uma classe de armazenamento com base na frequência esperada de acesso aos dados, pois o acesso aos dados gera custos mais altos. A classe de armazenamento padrão para dados em camadas é _Standard Storage_. A menos que seja alterada manualmente, todos os dados inativos mantêm a classe de armazenamento padrão. Para alterar a classe de armazenamento de dados inativos, consulte  https://docs.cloud.google.com/storage/docs/managing-lifecycles["Documentação do Google Cloud: gerenciando o ciclo de vida de objetos"^].
+
--
A classe de armazenamento para camadas de dados abrange todo o sistema, não é por volume.

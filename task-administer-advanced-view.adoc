...
* Gerenciamento de storage de objetos ONTAP S3: Os recursos de gerenciamento de storage de objetos ONTAP S3 no Cloud Volumes ONTAP estão disponíveis apenas no System Manager, e não no Console.
...
. Clique no ícone image:icon-action.png["Um ícone que mostra três pontos horizontais para ações adicionais"] do agente de Console necessário e selecione *Editar agente de Console*.
...
* https://docs.netapp.com/us-en/ontap/concepts/introducing-ontap-interfaces-concept.html["Interfaces de usuário do ONTAP"^]
* https://docs.netapp.com/us-en/ontap/concept_administration_overview.html["Aprenda sobre administração de clusters com ONTAP System Manager"^]

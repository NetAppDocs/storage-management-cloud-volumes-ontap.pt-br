...
O recurso WORM está disponível para uso com suas assinaturas do marketplace e com a licença Bring-Your-Own License (BYOL) sem custo adicional. Entre em contato com seu representante de vendas da NetApp para adicionar o WORM à sua licença atual.
...
Você pode usar um aplicativo para enviar arquivos para WORM via NFS ou CIFS, ou usar a CLI do ONTAP para enviar arquivos para WORM automaticamente. Você também pode usar um arquivo anexável WORM para reter dados que são gravados incrementalmente, como informações de log. Para obter informações, consulte https://docs.netapp.com/us-en/ontap/snaplock/commit-files-worm-state-manual-task.html#commit-files-to-worm-manually["Confirmar arquivos no WORM usando ONTAP SnapLock"^].

Após ativar o armazenamento WORM em um sistema Cloud Volumes ONTAP, você deve usar a ONTAP CLI para todo o gerenciamento do armazenamento WORM. Para obter instruções, consulte o link:https://docs.netapp.com/us-en/ontap/snaplock/index.html["Documentação ONTAP sobre SnapLock"^].

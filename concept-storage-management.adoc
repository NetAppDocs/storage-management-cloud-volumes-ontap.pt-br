...
* Se um agregado atingir o limite de capacidade e não puder suportar discos adicionais, o Console move automaticamente um volume desse agregado para um agregado com capacidade disponível ou para um novo agregado.
+
...
Se o administrador da organização ou da conta definir o *Modo de Gerenciamento de Capacidade* como manual, o Console informará que você deve tomar as medidas apropriadas para as decisões de capacidade. Os mesmos exemplos descritos no modo automático se aplicam ao modo manual, mas cabe a você aceitar as ações.

.Links relacionados
* link:task-manage-capacity-settings.html["Aprenda a modificar o modo de gerenciamento de capacidade"] .
* Para obter informações sobre provisionamento na AWS, consulte link:reference-configs-aws.html["Provisionamento de disco para agregados na AWS"].

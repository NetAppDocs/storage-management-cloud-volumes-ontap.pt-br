...
----
A função personalizada inclui as permissões por padrão. https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-azure.html["Exibir permissão do Azure para o agente do Console"^]
...
+
Para obter ajuda com a replicação de dados, consulte  https://docs.netapp.com/us-en/data-services-replication/task-replicating-data.html["Replicando dados de e para a nuvem"^].
...
. Clique no image:icon-action.png["Um ícone que mostra três pontos horizontais para ações adicionais"] ícone do agente do Console que gerencia o seu Cloud Volumes ONTAP.

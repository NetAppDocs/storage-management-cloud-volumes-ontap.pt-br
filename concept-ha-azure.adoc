...
Discos:: Os dados do cliente residem em blobs de página do Premium Storage. Cada nó tem acesso ao armazenamento do outro nó. Armazenamento adicional também é necessário para link:reference-configs-azure.html#azure-ha-pair["dados de inicialização, raiz e núcleo"].
...
NOTE: A partir do Cloud Volumes ONTAP 9.15.0P1, os blobs de páginas do Azure não são mais suportados para novas implantações de par de HA. Se você atualmente usa blobs de páginas do Azure em implantações de par de HA existentes, pode migrar para tipos de instância de VM mais recentes nas VMs da série Edsv4 e VMs da série Edsv5. link:reference-configs-azure.html#ha-pairs["Saiba mais sobre as configurações com suporte no Azure"].
...
NOTE: Se você escolher Discos Gerenciados SSD Premium para um ambiente que atenda aos critérios acima, o Console implantará automaticamente os Discos Gerenciados SSD Premium v2. Não é possível alternar para discos gerenciados Premium SSD v1.

Discos:: Os dados do cliente residem em discos gerenciados com armazenamento localmente redundante (LRS). Cada nó tem acesso ao armazenamento do outro nó. Armazenamento adicional também é necessário para link:reference-configs-azure.html#azure-ha-pair["dados de inicialização, raiz, raiz do parceiro, núcleo e NVRAM"].
...
Discos:: Os dados do cliente residem em discos gerenciados com armazenamento redundante de zona (ZRS). Cada nó tem acesso ao armazenamento do outro nó. Armazenamento adicional também é necessário para link:reference-configs-azure.html#azure-ha-pair["dados de inicialização, raiz, raiz do parceiro e núcleo"].
Contas de armazenamento:: Contas de armazenamento são usadas para implantações baseadas em disco gerenciado para manipular logs de diagnóstico e camadas para armazenamento de blobs.
...
NOTE: Cloud Volumes ONTAP usa balanceadores de carga em modo ativo-passivo para sondagens. No Azure, apenas o nó ativo responde a uma sondagem. O nó passivo pode aparecer como inativo com um X. Esse é o comportamento esperado e não é motivo de preocupação.

.Link relacionado
link:task-azure-high-availability-mode.html["Habilitar o modo de alta disponibilidade para o Cloud Volumes ONTAP no Azure"^]

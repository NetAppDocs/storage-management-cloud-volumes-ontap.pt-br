...
Durante a implantação do Cloud Volumes ONTAP, o campo de proxy do mediador da AWS aceita um endereço de servidor. Se você inserir um domínio totalmente qualificado (FQDN) em vez de um endereço IP, o FQDN não será resolvido, a menos que a VPC da AWS esteja configurada com um servidor DNS personalizado. Isso ocorre porque o DNS padrão fornecido pela AWS resolve apenas registros DNS públicos e hosts internos da AWS, e não pode resolver FQDNs externos, como servidores proxy de clientes. Se esse cenário se aplicar a você, você tem duas opções:

* Adicione o endereço IP do servidor proxy diretamente no campo do proxy mediador em vez do FQDN.
* Configure um conjunto de opções DHCP em sua VPC que inclua os endereços do seu servidor DNS personalizado. Isso permite que a AWS resolva o domínio totalmente qualificado quando você o inserir no campo do servidor proxy durante a implantação. Consulte a  https://docs.aws.amazon.com/vpc/latest/userguide/DHCPOptionSet.html["Documentação do AWS"^] para obter mais detalhes.
...
TIP: Procurando informações sobre o agente do NetApp Console? https://docs.netapp.com/us-en/console-setup-admin/reference-ports-aws.html["Exibir regras de grupo de segurança para o agente do Console"^]
...
* https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-aws.html["Exibir requisitos de rede para o agente do Console"^]

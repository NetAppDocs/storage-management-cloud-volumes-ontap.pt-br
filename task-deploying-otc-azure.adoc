...
---
= Implante Cloud Volumes ONTAP usando opções avançadas no Azure
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
+
** Você deve ter um(a)  https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-azure.html["Agente de console associado ao seu sistema"^].
...
. Se você for solicitado, https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-azure.html["criar um agente de console"^].
...
| Editar credenciais | Você pode escolher diferentes credenciais do Azure e uma assinatura do Azure diferente para usar com este o Cloud Volumes ONTAP. Você precisa associar uma assinatura do Azure Marketplace à assinatura do Azure selecionada para implantar um sistema o Cloud Volumes ONTAP com pagamento conforme o uso. https://docs.netapp.com/us-en/console-setup-admin/task-adding-azure-accounts.html["Aprenda como adicionar credenciais"^] 
|===
. *Serviços*: habilite ou desabilite os serviços individuais que você deseja ou não usar com o Cloud Volumes ONTAP.
+
** https://docs.netapp.com/us-en/data-services-data-classification/concept-cloud-compliance.html["Saiba mais sobre a NetApp Data Classification"^]
...
TIP: Se a conta do Azure que você está usando tiver o recurso https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-azure.html["permissões necessárias"^], o NetApp Console remove os recursos do Cloud Volumes ONTAP de um grupo de recursos em caso de falha na implantação ou exclusão.
...
| Use existente | Se você escolher um grupo de segurança existente, ele deverá atender aos requisitos do Cloud Volumes ONTAP. link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-azure.html#security-group-rules["Exibir o grupo de segurança padrão"^]. 
|===
...
NOTE: Se uma versão Release Candidate, General Availability ou patch mais recente estiver disponível para a versão selecionada, então o Console atualiza o sistema para essa versão ao criar o ambiente de trabalho. Por exemplo, a atualização ocorre se você selecionar Cloud Volumes ONTAP 9.16.1 P3 e 9.16.1 P4 estiver disponível. A atualização não ocorre de uma versão para outra—for example, de 9.15 para 9.16.
...
+
Para obter ajuda na escolha do tipo e tamanho do disco, consulte link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#size-your-system-in-azure["Dimensionando seu sistema no Azure"^].
...
+
Esta opção está disponível apenas para determinados tipos de VM. Para descobrir quais tipos de VM são compatíveis, consulte link:reference-configs-azure.html#ha-pairs["Configurações suportadas por licença para pares HA"].
...
+
Para mais informações, consulte link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#choose-a-volume-usage-profile["Compreendendo os perfis de uso de volume"^] e link:concept-data-tiering.html["Visão geral da hierarquização de dados"^].
...
** O tamanho do disco é para todos os discos no agregado inicial e para quaisquer agregados adicionais que o Console cria quando você usa a opção de provisionamento simples.  Você pode criar agregados que usam um tamanho de disco diferente usando a opção de alocação avançada.
+
Para obter ajuda na escolha do tamanho do disco, consulte link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#size-your-system-in-azure["Dimensione seu sistema no Azure"^].
...
+
link:reference-configs-azure.html#ha-pairs["Saiba mais sobre as configurações com suporte no Azure"^].
...
.Links relacionados
* link:task-planning-your-config-azure.html["Planejando sua configuração do Cloud Volumes ONTAP no Azure"^]
* link:task-deploy-cvo-azure-mktplc.html["Implantar o Cloud Volumes ONTAP no Azure a partir do Azure Marketplace"^]

...
* Zona de disponibilidade única (AZ) ou múltiplas zonas de disponibilidade/três zonas (recomendado).
+
...
Tradicionalmente, as configurações de Cloud Volumes ONTAP HA no Google Cloud utilizam um modelo não compartilhado, no qual uma instância mediadora é usada para gerenciar operações de failover ou monitorar a integridade do nó. Isso exige a criação de uma VM adicional no Google Cloud e dois conjuntos de discos para cada nó.
...
Acesso à instância:: Para Debian, o usuário padrão da nuvem é  `admin`. O Google Cloud cria e adiciona certificados para o usuário  `admin` quando o acesso SSH é solicitado por meio do Google Cloud Console ou linha de comando gcloud. Você pode especificar  `sudo` para obter privilégios de root.
...
=== Discos compartilhados para configurações de HA em uma única zona de disponibilidade

Cloud Volumes ONTAP 9.19.1 e versões posteriores suportam configuração de HA compartilhada para uma única AZ. Na arquitetura de HA compartilhada, ambos os nós podem compartilhar um disco, o que altera o método de criação do agregado. Em vez de criar dois discos para duas máquinas virtuais, um único disco é criado e anexado às máquinas virtuais. A HA compartilhada não precisa do espelhamento de disco.
...
Múltiplas zonas de disponibilidade (recomendado):: A implantação de uma configuração de HA em três zonas garante disponibilidade contínua de dados caso ocorra uma falha em uma zona.  Observe que o desempenho de gravação é um pouco menor quando comparado ao uso de uma única zona, mas é mínimo.
Zona de disponibilidade única:: Quando implantada em uma única zona de disponibilidade (AZ), uma configuração de HA do Cloud Volumes ONTAP utiliza uma política de posicionamento distribuído. Essa política garante que uma configuração de HA esteja protegida contra um ponto único de falha dentro da zona, sem a necessidade de usar zonas separadas para obter isolamento de falhas.
+
--
Este modelo de implantação reduz seus custos porque não há cobranças de saída de dados entre zonas.

== Como funciona o armazenamento em um par de HA

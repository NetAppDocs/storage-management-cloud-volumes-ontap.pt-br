...
Para mais informações, consulte link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-aws.html#requirements-for-ha-pairs-in-multiple-azs["Requisitos de rede da AWS para Cloud Volumes ONTAP HA em várias AZs"^].
...
NOTE: Cloud Volumes ONTAP suporta apenas estas configurações. Selecionar tipos de disco não suportados ou instâncias não qualificadas na configuração da AWS Local Zone pode resultar em falha na implantação. O armazenamento em camadas de dados no Amazon S3 não é suportado se o seu sistema Cloud Volumes ONTAP estiver em uma AWS Local Zone, pois o acesso aos buckets do Amazon S3 fora da Local Zone envolve maior latência e impacta as atividades do Cloud Volumes ONTAP.

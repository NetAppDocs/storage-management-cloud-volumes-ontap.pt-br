...
summary: Configure sua rede do Google Cloud para que os sistemas Cloud Volumes ONTAP possam operar corretamente. Você aprenderá sobre requisitos de rede, endereços IP privados, VPCs, sub-redes, espelhamento de pacotes, regras de firewall e configurações de alta disponibilidade para o CVO, além dos requisitos do agente do Console. 
---
...
https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["Revise as permissões de VPC compartilhadas necessárias abordadas na implantação do agente do Console"^]
...
O agente do Console também contata vários endpoints para operações diárias. Para obter informações sobre os endpoints, consulte  https://docs.netapp.com/us-en/console-setup-admin/task-install-connector-on-prem.html#step-3-set-up-networking["Exibir endpoints contatados pelo agente do Console"^] e  https://docs.netapp.com/us-en/console-setup-admin/reference-networking-saas-console.html["Preparar a rede para usar o Console"^].
...
TIP: Procurando informações sobre o agente do Console? https://docs.netapp.com/us-en/console-setup-admin/reference-ports-gcp.html["Exibir regras de firewall para o agente do Console"^]
...
=== Configuração do Infrastructure Manager em modo privado ou ambientes protegidos

Se você deseja implantar o Cloud Volumes ONTAP 9.16.1 ou posterior no modo privado ou em um projeto protegido pelo VPC Service Controls, é necessário fazer algumas alterações de configuração para que o Cloud Volumes ONTAP possa usar o Google Cloud Infrastructure Manager como serviço de implantação em vez do Deployment Manager, que o Google eventualmente descontinuará.

.Antes de começar
* Certifique-se de que seu sistema Cloud Volumes ONTAP esteja na versão 9.16.1 ou posterior. Se não estiver, atualize seu sistema. Consulte  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-updating-ontap-cloud.html["Atualizar Cloud Volumes ONTAP"] para obter instruções.
* Certifique-se de que as APIs do Google Cloud estejam ativadas. Consulte  https://docs.netapp.com/us-en/console-setup-admin/task-install-agent-google-gcloud.html#step-5-enable-google-cloud-apis["Ativar APIs do Google Cloud"^].
* Certifique-se de que a API Cloud Build esteja ativada. Consulte  https://console.cloud.google.com/flows/enableapi?apiid=cloudbuild.googleapis.com&redirect=https://console.cloud.google.com/build/docs/private-pools/set-up-private-pool-environment["Ative a API Cloud Build aqui"^].
* Verifique se a conta de serviço do agente do Console possui todas as permissões padrão. Além disso, certifique-se de que a conta de serviço tenha as  `cloudbuild.workerpools.get` e  `cloudbuild.workerpools.list` permissões. Consulte  https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html#service-account-permissions["Permissões do Google Cloud para o agente do Console"^].


.Passos
. Crie um pool de workers privado com essa configuração na mesma região das implantações do Cloud Volumes ONTAP. Para obter informações sobre como criar um pool de workers privado, consulte  https://docs.cloud.google.com/build/docs/private-pools/create-manage-private-pools#creating_a_new_private_pool["Documentação do Google Cloud: criar e gerenciar pools privados"^] e  https://cloud.google.com/build/pricing["Preços do Google Cloud Build"^].
+
Se você implantar o Cloud Volumes ONTAP em um projeto protegido por VPC Service Controls, crie o pool de workers privado no mesmo projeto que o agente do Console. O pool de workers deve estar em execução. O pool de workers público padrão do Cloud Build do Google Cloud não pode ser executado dentro de um perímetro de serviço.

+
O pool de trabalhadores deve ter a seguinte configuração:

+
** Tipo de máquina: e2-medium
** Tamanho do disco: 100 GB
** Atribuir IP externo: false
** Rede: padrão ou privada.
** A sub-rede configurada para acessar https://cloud.google.com/apis["APIs do Google Cloud"^]. Execute estas etapas para garantir que a sub-rede possa acessar as APIs do Google:
+
... Certifique-se de que o "Acesso privado do Google" esteja ativado para a subnet.
... Acesse *VPC Network level > Aba Private Service Access > Allocated IP ranges for services*.
... Selecione *Allocate IP range* e aloque o intervalo de IP interno para a conexão privada com o Google Compute Service.
... Em *Conexão privada com serviços*, selecione *Criar conexão*.
... Selecione *Connected service producer = Google Cloud Platform*.
... Atribua a alocação para o intervalo de IP de conexão privada que você criou na etapa anterior.




. Implante esse pool de workers e mantenha-o em execução para o gerenciamento do Cloud Volumes ONTAP. O Google Cloud usa esse pool de workers para executar todas as operações do Terraform em um ambiente isolado.
. Ao implantar Cloud Volumes ONTAP no modo privado, selecione o nome deste pool de workers no campo *GCP Worker Pool*. Consulte link:task-deploying-gcp.html["Inicie o Cloud Volumes ONTAP no Google Cloud"] para obter instruções.
+
Se você implantar o Cloud Volumes ONTAP em um projeto protegido pelo VPC Service Controls, crie o sistema Cloud Volumes ONTAP usando a API e inclua o parâmetro  `workerPool` no corpo da solicitação de criação.

+

NOTE: Atualmente, não há suporte para inclusão de um pool de workers ao implantar o Cloud Volumes ONTAP usando a interface do usuário do Console. Para projetos protegidos por Controles de Serviço de VPC, a API é a única maneira compatível de implantar o Cloud Volumes ONTAP.
...
* https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["Exibir requisitos de rede para o agente do Console"^]
...
Para obter informações sobre como configurar servidores proxy para o agente do Console, consulte o  https://docs.netapp.com/us-en/console-setup-admin/task-configuring-proxy.html["Configurar um agente de console para usar um servidor proxy"^].

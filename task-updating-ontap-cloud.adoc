...
A versão do Cloud Volumes ONTAP para a qual você pode atualizar depende da versão que você está executando atualmente. Cada versão genérica ou de patch em um lançamento nas tabelas a seguir representa a versão base disponível para atualização. Para obter detalhes sobre os patches disponíveis, consulte a  https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-new.html["notas de lançamento versioned"^] de cada lançamento.

ifdef::aws[]



==== Caminhos de atualização suportados para AWS
...
==== Caminhos de atualização com suporte para o Azure
...
==== Caminhos de atualização suportados para Google Cloud
...
O Cloud Volumes ONTAP deve estar registrado junto ao Suporte da NetApp para que você possa atualizar o software usando qualquer um dos métodos descritos nesta página. Isso se aplica tanto ao pagamento conforme o uso (PAYGO) quanto ao Bring-Your-Own License (BYOL). Você precisará link:task-registering.html["registrar manualmente os sistemas PAYGO"], enquanto os sistemas BYOL já vêm registrados por padrão.
...
. No bloco de agregados desejado, clique no ícone image:icon-action.png["Um ícone que mostra três pontos horizontais para ações adicionais"] e selecione *Exibir detalhes do agregado*.
+
...
. No sistema, clique no ícone image:icon-action.png["Um ícone que mostra três pontos horizontais para ações adicionais"] e, em seguida, clique em *Atualizar Cloud Volumes ONTAP*.

...
Para obter informações sobre como configurar servidores proxy, consulte o  https://docs.netapp.com/us-en/console-setup-admin/task-configuring-proxy.html["Configurar o agente do Console para usar um servidor proxy"^].
...
Cloud Volumes ONTAP oferece suporte  https://learn.microsoft.com/en-us/azure/virtual-network/virtual-network-encryption-overview["Criptografia da Azure Virtual Network (VNet)"^]à criptografia do tráfego entre máquinas virtuais (VMs) dentro de uma VNet ou entre VNets emparelhadas. Esse recurso é configurado na camada VNet do Azure e é independente da topologia do Cloud Volumes ONTAP (nó único ou HA).
...
https://learn.microsoft.com/en-us/azure/virtual-network/accelerated-networking-overview?tabs=NetworkManager["Documentação do Azure: criptografia VNet e Accelerated Networking"^].
...
O Console cria grupos de segurança do Azure que incluem as regras de entrada e saída para que o Cloud Volumes ONTAP funcione corretamente https://docs.netapp.com/us-en/console-setup-admin/reference-ports-azure.html["Exibir regras de grupo de segurança para o agente do Console"^].
...
Não recomendamos modificar os grupos de segurança predefinidos nem usar grupos de segurança personalizados. No entanto, se for imprescindível, observe que o processo de implantação exige que o Cloud Volumes ONTAP tenha acesso total à sua própria sub-rede. Após a conclusão da implantação, se você decidir modificar o grupo de segurança de rede, certifique-se de manter as portas do cluster e as portas da rede de HA abertas, pois a rede de HA depende da comunicação irrestrita entre os dois nós. Além disso, os endereços IP dos nós são definidos automaticamente durante a implantação. Isso garante uma comunicação perfeita dentro do cluster do Cloud Volumes ONTAP (comunicação qualquer para qualquer entre os nós).
...
* https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-azure.html["Exibir requisitos de rede para o agente do Console"^]

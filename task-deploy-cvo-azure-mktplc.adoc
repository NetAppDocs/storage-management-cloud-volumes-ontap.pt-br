...
summary:  
---
= Implante o Cloud Volumes ONTAP a partir do Azure Marketplace
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Você pode usar a implantação direta do Azure Marketplace para implantar o Cloud Volumes ONTAP de forma rápida e fácil. No Azure Marketplace, você pode implantar o Cloud Volumes ONTAP com apenas alguns cliques e explorar seus principais recursos e funcionalidades em seu ambiente.
...
.Sobre esta tarefa
O Cloud Volumes ONTAP implantado usando a implantação direta do Azure Marketplace possui estas propriedades. Observe que os recursos de uma instância independente implantada por meio do Azure Marketplace mudam quando ela é descoberta no NetApp Console.

* A versão mais recente do Cloud Volumes ONTAP (9.16.1 ou posterior).
* Uma licença gratuita para o Cloud Volumes ONTAP é fornecida para até 500 GiB de capacidade provisionada, sem suporte da NetApp e sem data de expiração. Capacidade acima de 500 GiB requer um direito de licenciamento válido e pode gerar cobranças. Para acessar a experiência completa do Cloud Volumes ONTAP e os recursos avançados de gerenciamento, conecte seu sistema Cloud Volumes ONTAP ao Console. A descoberta e o gerenciamento pelo Console exigem uma assinatura válida do Azure Marketplace associada à sua conta do Console.
...
.Antes de começar
* Se você planeja descobrir e gerenciar o sistema implantado no Console, certifique-se de que a conta do Console esteja associada a uma assinatura válida do Azure Marketplace. A descoberta do sistema no Console requer uma assinatura válida.
...
.Passos
. No site do Azure Marketplace, pesquise por produtos da NetApp.
...
Os sistemas Cloud Volumes ONTAP implantados diretamente pelo Azure Marketplace não incluem suporte da NetApp. Se surgir algum problema durante a implantação, você pode solucioná-lo por conta própria.

.Passos
. No site do Azure Marketplace, acesse *Boot diagnostics > Serial log*.
...
Você pode descobrir os sistemas Cloud Volumes ONTAP que você implantou usando a implantação direta do Azure Marketplace e gerenciá-los na página *Sistemas* no Console. O agente do Console descobre os sistemas, os adiciona, aplica as licenças necessárias e desbloqueia todos os recursos do Console para esses sistemas. A configuração original de HA em uma única AZ com Discos Gerenciados PSSD v2 é mantida, e o sistema é registrado na mesma assinatura do Azure e no mesmo grupo de recursos da implantação original.

.Sobre esta tarefa
Ao detectar os sistemas Cloud Volumes ONTAP implantados usando a implantação direta do Azure Marketplace, o agente do Console executa as seguintes tarefas:
...
.Antes de começar
Certifique-se de que a implantação esteja concluída no Azure Marketplace. O agente do Console só poderá descobrir os sistemas quando a implantação estiver concluída e eles estiverem disponíveis para descoberta.
...
CAUTION: Durante a descoberta, você pode ver mensagens de falha, mas pode ignorá-las até que o processo de descoberta seja concluído. Não modifique as configurações do Cloud Volumes ONTAP geradas pelo sistema no portal do Azure Marketplace durante a descoberta, especialmente as tags do sistema. Quaisquer alterações feitas nessas configurações podem levar a um comportamento inesperado do sistema.
...
* https://docs.netapp.com/us-en/console-licenses-subscriptions/task-manage-subscriptions.html["Adicionar uma nova assinatura do marketplace"^]

...
https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-limits-gcp.html["Limites de armazenamento para Cloud Volumes ONTAP no Google Cloud"^]
...
Tipo de máquina:: Consulte os tipos de máquinas suportados na link:reference-configs-gcp.html["Notas de versão do Cloud Volumes ONTAP"^] e, em seguida, verifique os detalhes de cada tipo de máquina no Google. Adeque os requisitos da sua carga de trabalho ao número de vCPUs e à memória do tipo de máquina. Observe que cada núcleo de CPU aumenta o desempenho da rede.
+
--
...
--
Tipos de disco:: Ao criar volumes para o Cloud Volumes ONTAP, você precisa escolher o armazenamento em nuvem subjacente que o Cloud Volumes ONTAP usa para um disco.  O tipo de disco pode ser qualquer um dos seguintes:
+
--
...
--
Tamanho do disco:: Você precisa escolher um tamanho de disco inicial ao implantar um sistema Cloud Volumes ONTAP .  Depois disso, você pode deixar que o NetApp Console gerencie a capacidade de um sistema para você, mas se quiser criar agregados sozinho, esteja ciente do seguinte:
+
--
...
* link:reference-configs-gcp.html#google-cloud-single-node["Visualize os discos padrão para dados do sistema Cloud Volumes ONTAP no Google Cloud"].
* https://docs.cloud.google.com/docs/quotas/overview["Documentação do Google Cloud: Visão geral das quotas de nuvem"^]
+
...
TIP: O agente do Console também requer um disco de sistema. https://docs.netapp.com/us-en/console-setup-admin/reference-connector-default-config.html["Exibir detalhes sobre a configuração padrão do agente do Console"^].

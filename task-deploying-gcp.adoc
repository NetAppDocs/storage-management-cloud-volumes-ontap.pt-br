...
+
** Você deve ter um(a)  https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["Agente de console associado ao seu sistema"^].
** https://docs.netapp.com/us-en/console-setup-admin/concept-connectors.html["Você deve estar preparado para deixar o agente do Console em execução o tempo todo"^].
...
. Se você for solicitado https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["criar um agente de console"^].
...
| Nome da conta de serviço | Se você planeja usar link:concept-data-tiering.html["hierarquização de dados"] ou  https://docs.netapp.com/us-en/data-services-backup-recovery/concept-backup-to-cloud.html["NetApp Backup and Recovery"^] com o Cloud Volumes ONTAP, então você precisa habilitar a *Conta de Serviço* e selecionar uma conta de serviço que tenha a função predefinida de Administrador de Armazenamento. link:task-creating-gcp-service-account.html["Aprenda a criar uma conta de serviço"^]. 
...
. *Localização e conectividade*: Selecione a região e a zona do Google Cloud para o seu sistema, escolha uma política de firewall e confirme a conectividade de rede com o storage do Google Cloud para o tiering de dados.
+
A tabela a seguir descreve os campos para os quais você pode precisar de orientação:
...
+
Para obter ajuda na escolha do tipo e tamanho do disco, consulte link:task-planning-your-config-gcp.html#size-your-system-in-google-cloud["Dimensione seu sistema no Google Cloud"^].
...
+
Saiba mais sobre link:reference-configs-gcp.html#flash-cache-support["Cache Flash"] e link:concept-write-speed.html["velocidade de escrita"].

+

NOTE: Alta velocidade de gravação e uma unidade máxima de transmissão (MTU) maior, de 8.896 bytes, estão disponíveis por meio da opção de velocidade de gravação *Alta*. Além disso, a MTU maior de 8.896 bytes exige a seleção de VPC-1, VPC-2 e VPC-3 para a implantação. Para mais informações sobre VPC-1, VPC-2 e VPC-3, consulte  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-gcp.html#requirements-for-the-console-agent["Regras para VPC-1, VPC-2 e VPC-3"^].
...
** Para obter ajuda na adição de uma conta do Google Cloud, consulte  https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["Configurando e adicionando contas do Google Cloud para camadas de dados com 9.6"^].
...
| Servidor NTP | Selecione *Usar Domínio do Active Directory* para configurar um servidor NTP usando o DNS do Active Directory. Se você precisar configurar um servidor NTP usando um endereço diferente, então deve usar a API. Para obter informações, consulte a  https://docs.netapp.com/us-en/console-automation/index.html["Documentação de automação do NetApp Console"^] para detalhes. Observe que você só pode configurar um servidor NTP ao criar um servidor CIFS. Não é possível configurá-lo depois que você cria o servidor CIFS. 
|===
...
. *Detalhes do sistema*: selecione um projeto, especifique um nome de cluster, opcionalmente selecione uma conta de serviço, opcionalmente adicione rótulos e, em seguida, especifique as credenciais.
+
A tabela a seguir descreve os campos para os quais você pode precisar de orientação:
...
| Projeto do Google Cloud e assinatura do Marketplace  a| 
Selecione o projeto do Google e a assinatura que você deseja usar para este sistema. Esta é a conta de serviço que você configurou para o Console. Para obter informações, consulte a link:https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["Documentos de configuração do console"^]. O Console usa o projeto para criar o sistema Cloud Volumes ONTAP e a assinatura para determinar a cobrança e o uso. Se você não vir nenhum projeto adicional na lista suspensa, então ainda não associou a conta de serviço a outros projetos. Acesse o Google Cloud Console, abra o serviço IAM e selecione o projeto. Adicione a conta de serviço com a função que você usa para o Console a esse projeto. Você precisará repetir esta etapa para cada projeto.

Clique em *Adicionar assinatura* para associar as credenciais selecionadas a uma assinatura.

Para criar um sistema Cloud Volumes ONTAP com pagamento conforme o uso, você precisa selecionar um projeto do Google Cloud associado a uma assinatura do Cloud Volumes ONTAP no Google Cloud Marketplace. Consulte o documento de configuração do Console: https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["Associando uma assinatura do marketplace às credenciais do Google Cloud"^].
...
| Nome da conta de serviço | Se você pretende usar os serviços link:concept-data-tiering.html["NetApp Cloud Tiering"] ou  https://docs.netapp.com/us-en/data-services-backup-recovery/concept-backup-to-cloud.html["Backup e Recuperação"^], precisa habilitar a opção *Conta de Serviço* e, em seguida, selecionar a Conta de Serviço que possui a função predefinida de Administrador de Armazenamento. 


| Etiquetas | Os rótulos são metadados para seus recursos do Google Cloud.  O Console adiciona os rótulos ao sistema Cloud Volumes ONTAP e aos recursos do Google Cloud associados ao sistema.  Você pode adicionar até quatro rótulos na interface do usuário ao criar um sistema e depois adicionar mais depois que ele for criado.  Observe que a API não limita você a quatro rótulos ao criar um sistema.  Para obter informações sobre rótulos, consulte https://cloud.google.com/compute/docs/labeling-resources["Documentação do Google Cloud: Rotulagem de recursos"^] . 


| Nome de usuário e senha do ONTAP | Estas são as credenciais para a conta de administrador do cluster Cloud Volumes ONTAP .  Você pode usar essas credenciais para se conectar ao Cloud Volumes ONTAP por meio do ONTAP System Manager ou do ONTAP CLI.  Mantenha o nome de usuário padrão _admin_ ou altere-o para um nome de usuário personalizado. 
|===
. *Serviços*: Selecione os serviços que você deseja usar neste sistema.  Para selecionar Backup e Recuperação ou usar o NetApp Cloud Tiering, você deve ter especificado a Conta de Serviço na etapa 3.
+
...
. *Topologia de implantação*: escolha várias zonas de disponibilidade (AZ) (recomendado) ou uma única AZ para a configuração de par de HA.
+
link:concept-ha-google-cloud.html["Saiba mais sobre modelos de implantação de HA"^] .

. *Configuração de rede e conectividade*: selecione as regiões e zonas relevantes. Selecione quatro VPCs diferentes para a configuração de HA, uma sub-rede em cada VPC e, em seguida, escolha uma política de firewall.
+

NOTE: Cloud Volumes ONTAP 9.19.1 e versões posteriores oferecem suporte link:concept-ha-google-cloud.html["Discos compartilhados para configurações de HA em uma única zona de disponibilidade"], tornando a instância do mediador opcional. Nessa configuração, você não precisa inserir uma zona de mediador, a VPC correspondente e a sub-rede para replicação de dados.
...
+
Se você escolher um dos pacotes, basta especificar um volume e, em seguida, revisar e aprovar a configuração. Os pacotes pré-configurados variam de acordo com a versão do Cloud Volumes ONTAP e os tipos de configuração selecionados. Por exemplo, se você selecionar uma implantação em uma única AZ para Cloud Volumes ONTAP 9.19.1 ou posterior, verá pacotes pré-configurados com máquinas da série C3 e discos Hyperdisk Balanced. Para versões anteriores e múltiplas AZs, você ainda pode selecionar máquinas n2-standard e discos Persistent Disks. Você pode modificar as configurações, como parâmetros de IOPS e taxa de transferência, com base nas necessidades da sua carga de trabalho.
...
NOTE: Alta velocidade de gravação e uma unidade máxima de transmissão (MTU) maior, de 8.896 bytes, estão disponíveis por meio da opção de velocidade de gravação *Alta* nos tipos de instância n2-standard-16, n2-standard-32, n2-standard-48 e n2-standard-64. Além disso, a MTU maior de 8.896 requer a seleção de VPC-1, VPC-2 e VPC-3 para a implantação. A alta velocidade de gravação e a MTU de 8.896 são dependentes do recurso e não podem ser desativadas individualmente em uma instância configurada. Para obter mais informações sobre VPC-1, VPC-2 e VPC-3, consulte  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-gcp.html#requirements-for-the-connector["Regras para VPC-1, VPC-2 e VPC-3"^].
...
| Servidor NTP | Selecione *Usar Domínio do Active Directory* para configurar um servidor NTP usando o DNS do Active Directory. Se você precisar configurar um servidor NTP usando um endereço diferente, então deve usar a API. Consulte a  https://docs.netapp.com/us-en/console-automation/index.html["Documentação de automação do NetApp Console"^] para obter detalhes. Observe que você só pode configurar um servidor NTP ao criar um servidor CIFS. Não é possível configurá-lo depois que você cria o servidor CIFS. 
|===
. *Perfil de uso, tipo de disco e política de camadas*: escolha se deseja habilitar recursos de eficiência de armazenamento e alterar a política de camadas de volume, se necessário.
+
Para mais informações, consultelink:task-planning-your-config-gcp.html#choose-a-volume-usage-profile["Escolha um perfil de uso de volume"^] ,link:concept-data-tiering.html["Visão geral da hierarquização de dados"^] , e https://kb.netapp.com/Cloud/Cloud_Volumes_ONTAP/What_Inline_Storage_Efficiency_features_are_supported_with_CVO#["KB: Quais recursos de eficiência de armazenamento em linha são suportados pelo CVO?"^]

...
Com base na sua configuração e tipo de licença, você pode criar várias VMs de armazenamento em um sistema de nó único ou em uma configuração de alta disponibilidade (par de HA) usando as APIs.
...
.Antes de começar
O agente do Console requer permissões específicas para criar VMs de armazenamento para pares de alta disponibilidade do Cloud Volumes ONTAP . As permissões necessárias estão incluídas em https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html["as políticas fornecidas pela NetApp"^] .

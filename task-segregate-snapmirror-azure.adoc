...
---
= Segregue o tráfego do SnapMirror no Cloud Volumes ONTAP no Azure
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
----
.. Salve o endereço IP privado. Este endereço IP, <new_added_nic_primary_addr>, é usado para criar uma LIF entre clusters em <<step-second,domínio de broadcast e LIF entre clusters para a nova NIC>>.
...
== [[step-second]]Etapa 2: Crie um novo IPspace, domínio de broadcast e LIF entre clusters para a nova NIC
...
----
. Para sistemas de nó único, a porta recém-adicionada é _e0b_. Para implantações de par de HA com discos gerenciados, a porta recém-adicionada é _e0d_. Para implantações de par de HA com blobs de página, a porta recém-adicionada é _e0e_. Use o nome do nó, não o nome da VM. Encontre o nome do nó executando  `node show`.
+
[source, cli]
----
...
. Crie um peering de cluster na origem usando o endereço IP do LIF do cluster de destino como `peer-addrs` .  Para pares HA, liste o endereço IP do LIF intercluster de destino para ambos os nós como `-peer-addrs` .
+
[source, cli]
----

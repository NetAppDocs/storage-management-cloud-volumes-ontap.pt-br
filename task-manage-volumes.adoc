...
.Passos
. Selecione a visualização do System Manager para redimensionar um volume por meio do ONTAP System Manager. Consulte link:task-administer-advanced-view.html#get-started-with-system-manager["Comece a usar o System Manager"].
...
TIP: O Console exibe essas mensagens de "Ação Necessária" quando um agregado atinge 90% da capacidade utilizada. Se o armazenamento em camadas de dados estiver ativado, as mensagens são exibidas quando um agregado atinge 80% da capacidade utilizada. Por padrão, 10% de espaço livre é reservado para o armazenamento em camadas de dados. link:task-tiering.html#change-the-free-space-ratio-for-data-tiering["Saiba mais sobre a proporção de espaço livre para camadas de dados"^].
...
.. No bloco de agregados, clique no ícone image:icon-action.png["Um ícone que mostra três pontos horizontais para ações adicionais"] e, em seguida, clique em *Ver detalhes do agregado*.
...
+
Para obter mais informações, consulte link:https://docs.netapp.com/us-en/data-services-replication/task-replicating-data.html["Replicando dados entre sistemas"^].
...
+
Para obter mais informações, consulte link:https://docs.netapp.com/us-en/data-services-replication/task-replicating-data.html#managing-data-replication-schedules-and-relationships["Gerenciando cronogramas e relacionamentos de replicação de dados"^].
...
+
.. No sistema Cloud Volumes ONTAP , clique na guia *Agregados*.
.. No bloco de agregado necessário, clique no ícone image:icon-action.png["Um ícone que mostra três pontos horizontais para ações adicionais"] e depois em *Ver detalhes do agregado* para visualizar a capacidade disponível (tamanho provisionado menos a capacidade do agregado utilizada).
+
...
+
.. Selecione o agregado, depois clique no ícone image:icon-action.png["Um ícone que mostra três pontos horizontais para ações adicionais"] > *Adicionar discos*.
